<template>
  <div class="relative">
    <div class="-mt-20 black-curve bg-no-repeat bg-cover h-20" />
    <div class="bg-hh-black flex items-center justify-center py-32">
      <div class="">
        <div class="relative">
          <div class="tracking-wide text-center font-sans text-5xl leading-12 font-bold tracking-tight text-white sm:text-6xl sm:leading-14">
            Sammen om et
            <br>
            skodfrit Danmark
          </div>
        </div>

        <div class="mt-24">
          <div class="max-w-xl mx-auto px-4 sm:px-6 lg:max-w-screen-xl lg:px-8">
            <div class="lg:grid lg:grid-cols-3 lg:gap-12">
              <div>
                <div class="flex items-center mx-auto justify-center h-56 w-56 rounded-md bg-hh-primary text-white">
                  <svg class="h-24 w-24" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" />
                  </svg>
                </div>
                <div class="mt-5">
                  <h5 class="text-center font-serif text-2xl leading-9 font-medium text-white">
                    Skod bliver smidt hvert år
                  </h5>
                </div>
              </div>
              <div class="mt-10 lg:mt-0">
                <div class="flex items-center mx-auto justify-center h-56 w-56  rounded-md bg-hh-primary text-white">
                  <svg class="h-24 w-24" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16H9m3 0h3" />
                  </svg>
                </div>
                <div class="mt-5">
                  <h5 class="text-center font-serif text-2xl leading-9 font-medium text-white">
                    Cigaretskod er akut giftige for organismer i salt og ferskvand
                  </h5>
                </div>
              </div>
              <div class="mt-10 lg:mt-0">
                <div class="flex items-center mx-auto justify-center h-56 w-56 rounded-md bg-hh-primary text-white">
                  <svg class="h-24 w-24" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
                  </svg>
                </div>
                <div class="mt-5">
                  <h5 class="text-center font-serif text-2xl leading-9 font-medium text-white">
                    Små børn kan blive forgiftede af at spise cigaretskod
                  </h5>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>
